...
unsigned char Rcon[15] = {0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40,
...
void KeyExpansion(const unsigned char* key, unsigned char* expKey) {
    ...
    int bytesGen = KEY_SIZE;
    int rconIter = 1;
    unsigned char temp[4];

    while (bytesGen < EXP_KEY_SIZE) {
        memcpy(temp, expKey + bytesGen - 4, 4);

        if (bytesGen % KEY_SIZE == 0) {
            RotWord(temp);
            SubWord(temp);
            temp[0] ^= Rcon[rconIter++];
        } else if (bytesGen % KEY_SIZE == 16) {
            SubWord(temp);
        }
        
        for (int i = 0; i < 4; ++i) {
            expKey[bytesGen] = expKey[bytesGen - KEY_SIZE] ^ temp[i];
            ++bytesGen;
        }
    }
}
...
void MixColumns(unsigned char* state) {
    ...
    for (int i = 0; i < 4; ++i) {
        int idx = 4 * i;
        tmp[idx]   = mul2[state[idx]] ^ mul3[state[idx+1]] ^ state[idx+2]   ^ state[idx+3];
        tmp[idx+1] = state[idx]   ^ mul2[state[idx+1]] ^ mul3[state[idx+2]] ^ state[idx+3];
        tmp[idx+2] = state[idx]   ^ state[idx+1]   ^ mul2[state[idx+2]] ^ mul3[state[idx+3]];
        tmp[idx+3] = mul3[state[idx]] ^ state[idx+1]   ^ state[idx+2]   ^ mul2[state[idx+3]];
    }
    memcpy(state, tmp, 16);
}
...
void AES_Encrypt(unsigned char* buf, const unsigned char* key) {
    ...
    unsigned char roundKey[EXP_KEY_SIZE];
    ...
    SubBytes(state);
    ShiftRows(state);
    AddRoundKey(state, roundKey + 16 * Nr);
...
vector<unsigned char> StringToBytes(const string& text) {
    ...
    for(char c : text) bytes.push_back((unsigned char)c);
    return bytes;
}

string BytesToHex(unsigned char* data, int len) {
    stringstream ss;
    for(int i = 0; i < len; ++i) {
        ss << hex << uppercase << setw(2) << setfill('0') << (int)data[i];
    }
    return ss.str();
}
...
string EncryptMessage(const string& msg, const unsigned char* key) {

    int origLen = msg.size();
    int padLen = ((origLen % 16) ? (origLen / 16+1) * 16 : origLen);

    unsigned char* buffer = new unsigned char[padLen];

    memset(buffer, 0, padLen);
    memcpy(buffer, msg.c_str(), origLen);

    for(int i = 0; i < padLen; i += 16) {
        AES_Encrypt(buffer + i, key);
    }

    string encryptedHex = BytesToHex(buffer, padLen);
...
//     for(int i=0; i<32; i++){
//             key[i]=static_cast<unsigned char>(userKey[i]);
//         }
...
//         string decrypted = DecryptMessage(text, key);
//         cout << decrypted;
//     }
